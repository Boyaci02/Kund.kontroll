...
import { Bell, X, CheckCheck, ClipboardList, UserPlus, CheckSquare } from "lucide-react"
...
export function NotificationPanel() {
  ...
  return (
    <div ref={panelRef} className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2">
      {/* Panel */}
      {open && (
        <div className="w-80 max-h-[480px] flex flex-col rounded-2xl border border-border bg-card shadow-2xl overflow-hidden">
          {/* Header */}
      ...
      >
        ...
        {unreadCount > 0 && !open && (
          <span className="absolute -top-1 -right-1 h-5 w-5 rounded-full bg-red-500 text-white text-[10px] font-bold flex items-center justify-center leading-none">
